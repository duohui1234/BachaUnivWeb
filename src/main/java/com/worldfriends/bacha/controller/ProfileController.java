...
public class ProfileController {
	...
	public String editSubmit(HttpSession session,
		...
		if(result.hasErrors())return "student/edit";
	...
	public String changepasswordSubmit(HttpSession session,
								       @Valid Password password,
								       BindingResult result) throws Exception {
		
		//유효성 검사 실패시 정보 수정 페이지로 돌아감
		if(result.hasErrors())return "student/changepassword";
		
		//수정 실패시 비밀번호 수정 페이지로 돌아감
		if(!service.changepassword(password)) {
			result.reject("updateFail", "기존 비밀번호가 틀렸습니다.");
			return "student/changepassword";
		}

		Student student = service.getStudent(password.getStudentNumber()); //수정된 정보 얻기 위해 다시 검색
